...
public class CourseNameValidatorTest {
	...
	public void testIsValid() {
		...
		try {
			assertFalse(fsm.isValid("A000AA"));
			fail();
		} catch (InvalidTransitionException e) {
		...
		try {
			assertFalse(fsm.isValid("A000A0"));
			fail();
		} catch (InvalidTransitionException e) {
		...
		try {
			...
			assertEquals(1, fsm.getLetterCount());
			assertEquals(3, fsm.getDigitCount());
		...
		// Test incomplete course name (too few digits (1) with suffix)
		try {
			assertFalse(fsm.isValid("AAA0A"));
		} catch (InvalidTransitionException e) {
			assertEquals("Course name must have 3 digits.", e.getMessage());
		}
		
		// Test incomplete course name (too few digits (2) with suffix)
		try {
			assertFalse(fsm.isValid("AAA00A"));
		} catch (InvalidTransitionException e) {
			assertEquals("Course name must have 3 digits.", e.getMessage());
		}
	}
	...
	public void testSetters() {
		CourseNameValidator fsm = new CourseNameValidator();
		fsm.setLetterCount(3);
		assertEquals(3, fsm.getLetterCount());
		fsm.setDigitCount(5);
		assertEquals(5, fsm.getDigitCount());
	}
